...
def fastani_run(inputdir,ani_cutoff,thread,redundant_genome):
    ...
    os.system('cd %s && ls > MAGs_list.txt && fastANI --ql MAGs_list.txt --rl MAGs_list.txt -o %s -t %s && rm MAGs_list.txt  > fastANI.log 2>&1' % (inputdir,output,min(len(os.listdir(inputdir))-2,thread)))
