...
def genome_download(krakenfile,cutoff,input_file,thread,kraken_result,complete,download_need,krakenfile_donor):
    HGT_check=kraken_result.replace('kraken_result','ref_genome')
    pathdownload=os.path.join(HGT_check,'',krakenfile)
    print('pathdownload:',pathdownload,'krakenfile:',krakenfile,'krakenfile_donor:',krakenfile_donor)
    ...
    donortaxid_result=kraken2taxid(krakenfile_donor,cutoff,kraken_result)
    print(kraken2taxid_result,'donortaxid_result:',donortaxid_result)
    if 'S' in kraken2taxid_result:
        if download_need=='need':
            print('------Download Species genome...------')
            if kraken2taxid_result['S'] == 'Escherichia coli':
                os.system('ncbi-genome-download bacteria --output-folder %s -M reference -F fasta -l %s --flat-output -g "%s" -P -p %s -r 5' % (pathdownload,complete,kraken2taxid_result['S'],thread))
            else:
                os.system('ncbi-genome-download bacteria --output-folder %s -F fasta -l %s --flat-output -g "%s" -P -p %s -r 5' % (pathdownload,complete,kraken2taxid_result['S'],thread))
    if 'G' in kraken2taxid_result:
        if download_need=='need':
            print('------Download Genus genome...------')
            os.system('ncbi-genome-download bacteria --output-folder %s -F fasta -M type -l all --flat-output -g "%s" -P -p %s -r 5' % (pathdownload,kraken2taxid_result['G'],thread))
        ...
        for gz in os.listdir(pathdownload):
            if '.gz' in gz:
                os.system('cd %s && gunzip  -f %s' % (pathdownload,gz))
        os.system('cp %s %s' % (pathtarget1,pathtarget2))
        return (kraken2taxid_result,donortaxid_result)
    else:
